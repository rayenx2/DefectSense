...
from defectsense.utils import get_logger, setup_logging
...
def test_exporter_loads_stats_pth_and_wraps(tmp_path, make_stats, patch_extractor):
    ...
    from defectsense.export import ModelExporter, _ExportWrapper

    exp = ModelExporter(
        model_path=pth_path, output_dir=tmp_path, logger=logger, device="cpu"
    )
