"""
Complete backward compatibility layer for anodet -> defectsense migration.

This module provides bulletproof backward compatibility for existing code that imports anodet.
All functionality has been moved to the 'defectsense' package.
...
Usage:
    ...
    # New way (recommended)
    import defectsense
    from defectsense.utils import adaptive_gaussian_blur
    from defectsense.inference.model.wrapper import ModelWrapper
    model = defectsense.Padim()

This compatibility layer will be removed in DefectSense 4.0.0
"""
...
warnings.warn(
    ...
    "═══════════════════════════════════════════════════════════\n"
    "The 'anodet' package has been renamed to 'defectsense'.\n"
    "\n"
    "Please update your imports:\n"
    "  ❌ OLD: import anodet\n"
    "  ✅ NEW: import defectsense\n"
    "\n"
    "  ❌ OLD: from anodet import Padim\n"
    "  ✅ NEW: from defectsense import Padim\n"
    "\n"
    "  ❌ OLD: from anodet.utils import get_logger\n"
    "  ✅ NEW: from defectsense.utils import get_logger\n"
    "\n"
    "Migration timeline:\n"
    "  • Now - v3.x: Both packages work (with warnings)\n"
    "  • v4.0.0: Legacy 'anodet' support will be removed\n"
    "\n"
    "Migration guide: https://github.com/rayenx2/defectsense#migration\n"
    "═══════════════════════════════════════════════════════════\n",
    DeprecationWarning,
    stacklevel=2,
)
...
try:
    ...
    # Core models
    # Import ALL public APIs from defectsense to make them available via anodet
    from defectsense import *
    from defectsense import (
        AnodetDataset,
        MVTecDataset,
        Padim,
    ...
    from defectsense.utils import (  # Image processing utilities; Logging and configuration; The specific function that was causing the import error; File utilities; Gaussian blur utilities
    ...
    for module_name in main_modules:
        try:
            ...
            defectsense_module = __import__(
                f"defectsense.{module_name}", fromlist=[module_name]
            )
            # Map it in sys.modules
            sys.modules[f"anodet.{module_name}"] = defectsense_module
    ...
    try:
        import defectsense.datasets

        sys.modules["anodet.datasets"] = defectsense.datasets
        ...
        for sub in datasets_submodules:
            try:
                submodule = __import__(f"defectsense.datasets.{sub}", fromlist=[sub])
    ...
    try:
        import defectsense.inference

        sys.modules["anodet.inference"] = defectsense.inference
        print("✓ Mapped anodet.inference")

        # Map inference.model
        try:
            import defectsense.inference.model

            sys.modules["anodet.inference.model"] = defectsense.inference.model
            print("✓ Mapped anodet.inference.model")

            # Map inference.model.wrapper (THIS IS WHAT train.py NEEDS)
            try:
                import defectsense.inference.model.wrapper

                sys.modules["anodet.inference.model.wrapper"] = (
                    defectsense.inference.model.wrapper
                )
            ...
            try:
                import defectsense.inference.model.backends

                sys.modules["anodet.inference.model.backends"] = (
                    defectsense.inference.model.backends
                )
                ...
                for backend in backend_modules:
                    try:
                        backend_module = __import__(
                            f"defectsense.inference.model.backends.{backend}",
                            fromlist=[backend],
                        )
        ...
        try:
            import defectsense.inference.modelType

            sys.modules["anodet.inference.modelType"] = defectsense.inference.modelType
    ...
    try:
        import defectsense.visualization

        sys.modules["anodet.visualization"] = defectsense.visualization
        ...
        for viz_module in viz_submodules:
            try:
                viz_submodule = __import__(
                    f"defectsense.visualization.{viz_module}", fromlist=[viz_module]
                )
    ...
    try:
        import defectsense.sampling_methods

        sys.modules["anodet.sampling_methods"] = defectsense.sampling_methods
        ...
        for sub in sampling_submodules:
            try:
                submodule = __import__(
                    f"defectsense.sampling_methods.{sub}", fromlist=[sub]
                )
    ...
    __version__ = getattr(defectsense, "__version__", "3.0.0")
    __author__ = getattr(defectsense, "__author__", "Rayen Lassoued")
    __email__ = getattr(defectsense, "__email__", "")

    print("🎉 Backward compatibility mapping completed successfully!")
    print("🔧 All anodet imports should now work with defectsense backend.")

except ImportError as e:
    print(f"❌ FATAL: Failed to import from defectsense: {e}")
    raise ImportError(
        "Failed to import from 'defectsense' package. "
        f"Please ensure DefectSense is properly installed: {e}"
    ) from e
...
if hasattr(sys, "ps1"):  # Interactive session
    ...
    print("   Consider updating to 'defectsense' for the best experience!")
    print(
        "   Quick migration: https://github.com/rayenx2/defectsense#migration\n"
    )
